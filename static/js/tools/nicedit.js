...
var nicUploadButton = nicEditorAdvancedButton.extend({
    ...
    uploadFile: function() {
        ...
        D.link = data;
        D.width = "30px";
        this.onUploaded(D, B.type);

    },
    onUploaded: function(B, type) {
        this.removePane();
        if(B.link.length > 1300000){
        	this.onError("图片过大, 请压缩至1MB以内！");
        	return false;
        }
        var D = "data:"+ type +";base64,"+B.link;
        if (!this.im) {
            this.ne.selectedInstance.restoreRng();
